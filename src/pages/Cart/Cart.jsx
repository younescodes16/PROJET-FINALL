...
export default function Cart() {
  ...
  const handleGenerateInvoice = () => {
    ...
    cart.forEach((product, index) => {
      ...
      doc.text(
        `${(product.quantity * product.price).toFixed(2)} DA`,
    ...
    doc.text(
      `${total.toFixed(2)} DA`,
      startX + columnWidths.reduce((a, b) => a + b, 0) - 30,
      totalY
    );
    ...
    <>
      <div className="cart-image">
        ...
        <div className="btn-cart">
          ...
          <Link to="/paiment">
            <button className="paiment">PAIMENT</button>
          </Link>
          <button className="facture" onClick={handleGenerateInvoice}>
            FACTURE
          </button>
        </div>
      </div>
